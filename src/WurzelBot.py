...
class WurzelBot(object):
    ...
    def __getAllFieldIDsFromFieldIDAndSizeAsString(self, fieldID, sx, sy):
        ...
        if sx == "1" and sy == "1":
            return str(fieldID)
        if sx == "2" and sy == "1":
            return str(fieldID) + "," + str(fieldID + 1)
        if sx == "1" and sy == "2":
            return str(fieldID) + "," + str(fieldID + 17)
        if sx == "2" and sy == "2":
            return (
                str(fieldID)
                + ","
    ...
    def writeMessagesIfMailIsConfirmed(self, recipients, subject, body):
        ...
        if self.user.is_mail_confirmed():
            try:
                self.messenger.writeMessage(
                    self.user.get_username(), recipients, subject, body
                )
            except:
    ...
    def getEmptyFieldsOfGardens(self):
        ...
        try:
            ...
            self.__logBot.error(
                f"Could not determinate empty fields from garden {garden.getID()}."
            )
        return emptyFields
    ...
    def getGrowingPlantsInGardens(self):
        ...
        try:
            ...
            self.__logBot.error(
                "Could not determine growing plants of garden "
                + str(garden.getID())
                + "."
            )
    ...
    def sell_to_wimps(
        self,
        buy_from_shop: bool = False,
        minimal_balance: int = 500,
        method: str = "loss",
        max_amount: int = 100,
        max_loss_in_percent: int = 33,
    ):
        ...
        for wimps in wimps_data:
            for wimp, products in wimps.items():
                if not self.check_wimps_profitable(
                    products, method, max_amount, max_loss_in_percent
                ):
                ...
                check, stock_list = self.check_wimps_required_amount(
                    products[1], stock_list, minimal_balance, buy_from_shop
                )
                if not check:
                    continue
                ...
                for id, amount in products[1].items():
                    ...
                    npc_price += (
                        self.product_data.get_product_by_id(id).get_price_npc() * amount
                    )
        ...
        if counter > 0:
            ...
            print(
                f"Sold to {counter} wimps for {rewards:.2f} wT (equals {(rewards/npc_price):.2%} of Shop-price: {npc_price:.2f} wT)"
            )
    ...
    def check_wimps_required_amount(
        self, products, stock_list, minimal_balance, buy_from_shop: bool = False
    ):
        ...
        for id, amount in products.items():
            ...
            min_stock = max(
                self.note.get_min_stock(),
                self.note.get_min_stock(product.get_name()),
                minimal_balance,
            )
    ...
    def getNextRunTime(self):
        ...
        print(
            f"Next time water/harvest: {human_time.strftime('%d/%m/%y %H:%M:%S')} ({min(garden_time)})"
        )
    ...
    def getWeedFieldsOfGardens(self):
        ...
        try:
            ...
            self.__logBot.error(
                f"Could not determinate weeds on fields of garden {garden.getID()}."
            )
    ...
    def growVegetablesInGardens(self, productName, amount=-1):
        ...
        if amount == -1 or amount > self.stock.get_stock_by_product_id(
            product.get_id()
        ):
        ...
        for garden in self.gardens:
            planted += garden.grow(
                product.get_id(), product.get_sx(), product.get_sy(), remainingAmount
            )
    ...
    def get_ordered_stock_list(self):
        ...
        for productID in self.stock.get_ordered_stock_list():
            orderedList += str(
                self.product_data.get_product_by_id(productID).get_name()
            ).ljust(20)
    ...
    def getLowestVegetableStockEntry(self):
        ...
        for productID in self.stock.get_ordered_stock_list():
            if (
                not self.product_data.get_product_by_id(productID).is_vegetable()
                or not self.product_data.get_product_by_id(productID).is_plantable()
            ):
                continue
    ...
    def getLowestSingleVegetableStockEntry(self):
        ...
        for productID in self.stock.get_ordered_stock_list():
            if (
                not self.product_data.get_product_by_id(productID).is_vegetable()
                or not self.product_data.get_product_by_id(productID).is_plantable()
                or not self.product_data.get_product_by_id(productID).get_name()
                in self.product_data.get_single_field_vegetable_list()
            ):
                continue
    ...
    def getLowestWaterPlantStockEntry(self):
        ...
        for productID in self.stock.get_ordered_stock_list():
            if (
                not self.product_data.get_product_by_id(productID).is_water_plant()
                or not self.product_data.get_product_by_id(productID).is_plantable()
            ):
                continue
    ...
    def infinityQuest(self, MINwt):
        ...
        if self.user.get_level() > 23 and self.user.get_bar() > MINwt:
            ...
            if int(questnr) <= 500:
                for item in self.__HTTPConn.initInfinityQuest()["questData"][
                    "products"
                ]:
                    # print(item)
                    product = item["pid"]
                    ...
                    try:
                        self.__HTTPConn.sendInfinityQuest(
                            questnr, product.get_id(), needed
                        )
                    except:
                        pass
    ...
    def send_bees(self, tour: int):
        ...
        for key, value in honey_count.items():
            self.__logBot.info(
                f"Collected {value} {self.product_data.get_product_by_id(key).get_name()}."
            )
    ...
    def change_all_hives_types(self, product_name: str):
        if self.honey:
            self.honey.change_all_hives_types(
                self.product_data.get_product_by_name(product_name).get_id()
            )
